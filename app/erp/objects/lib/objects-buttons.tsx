...
import { updateObject } from './objects-actions';
import Link from 'next/link';
import { Object } from '@/app/lib/definitions';

export function CreateObject({ readonly }: { readonly: boolean; }) {
  ...
  return (
    ...
    >
      <span className="hidden md:block">Создать Объект</span>
      <PlusIcon className="h-5 md:ml-4" />
    </Link>
  );
}

export function BtnUpdateObject({ object }: { object: Object }) {
  const updateObjectWithData = updateObject.bind(null, object);
  return (
    <form action={updateObjectWithData} className="w-full">
      <button
        type="submit"
...
export function BtnEditObjectLink({ id }: { id: string }) {
