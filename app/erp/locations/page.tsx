...
export default async function Page(props: {
  ...
  const totalPages = await fetchLocationsPages(query, current_sections);

  return (
    <>
      <div className="w-full">
        <div className="flex w-full items-center justify-between">
          <h1 className={`${lusitana.className} text-2xl`}>Локации</h1>
        </div>
        <div className="mt-4 flex items-center justify-between gap-2 md:mt-8">
          <Search placeholder="Найти локацию..." initialQuery={query}  />
          <CreateLocation readonly={readonly_permission} />
        </div>
