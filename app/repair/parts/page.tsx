...
export default async function Page(props: {
  ...
  const parts = {};
  const readonly_permission = checkReadonly(userPermissions, parts, pageUser.id);
  ...
  const totalPages = await fetchPartsPages(query, current_sections);
    ...
    <>
      ...
      <div className="w-full">
        <div className="flex w-full items-center justify-between">
          <h1 className={`${lusitana.className} text-2xl`}>Запчасти</h1>
        </div>
        <div className="mt-4 flex items-center justify-between gap-2 md:mt-8">
          <Search placeholder="Найти запчасть..." initialQuery={query}  />
